...
void updateSpeed() {
  ...
  for (int i = 0; i < halfResolution; ++i) {
    if (lidarValues[i] < THRESHOLD_DISTANCE)  // far obstacles are ignored
      leftObstacle += lidarValues[i];
    // near obstacle sensed on the right side
    int j = lms291Resolution - i - 1;
    if (lidarValues[j] < THRESHOLD_DISTANCE)
      rightObstacle += lidarValues[j];
  }
  ...
  // if (obstacle > OBSTACLE_THRESHOLD) {
    const double speedFactor = leftObstacle/rightObstacle;
    speeds[0] = MAX_SPEED * speedFactor;
    speeds[1] = MAX_SPEED / speedFactor;
  ...
  for (int i = 0; i < NMOTORS; ++i) {
    ...
    set_velocity_srv.request.value = 5;
