...
void updateSpeed() {
  ...
  for (int i = 0; i < NMOTORS; ++i) {
    ...
    set_velocity_client = n->serviceClient<webots_ros::set_float>(std::string("Create/") + std::string(motorNames[i]) +
...
void quit(int sig) {
  ROS_INFO("User stopped the 'Create' node.");
...
int main(int argc, char **argv) {
  ...
  // create a node named 'Create' on ROS network
  ros::init(argc, argv, "Create", ros::init_options::AnonymousName);
  ...
  timeStepClient = n->serviceClient<webots_ros::set_int>("Create/robot/time_step");
  ...
  for (int i = 0; i < NMOTORS; ++i) {
    ...
    set_position_client = n->serviceClient<webots_ros::set_float>(std::string("Create/") + std::string(motorNames[i]) +
  ...
  set_GPS_client = n->serviceClient<webots_ros::set_int>("Create/gps/enable");
  GPS_srv.request.value = 32;
  if (set_GPS_client.call(GPS_srv) && GPS_srv.response.success) {
    sub_GPS = n->subscribe("Create/gps/values", 1, GPSCallback);
  ...
  set_inertial_unit_client = n->serviceClient<webots_ros::set_int>("Create/inertial_unit/enable");
  inertial_unit_srv.request.value = 32;
  if (set_inertial_unit_client.call(inertial_unit_srv) && inertial_unit_srv.response.success) {
    sub_inertial_unit = n->subscribe("Create/inertial_unit/quaternion", 1, inertialUnitCallback);
  ...
  set_accelerometer_client = n->serviceClient<webots_ros::set_int>("Create/accelerometer/enable");
  ...
  set_camera_client = n->serviceClient<webots_ros::set_int>("Create/camera/enable");
  ...
  set_gyro_client = n->serviceClient<webots_ros::set_int>("Create/gyro/enable");
